...
// ---------------------------------------------------------------------------
// Differentiation testing of the trait seam (TODO #1)
// ---------------------------------------------------------------------------

#[tokio::test]
async fn delivery_routes_review_through_self_not_ps() {
    // The trait abstraction is "real" only if swapping the DeliveryController's
    // own review handle changes behavior while the PipelineState slot keeps
    // mock_A. Proves show_review routes through self.review(), not ps.review().
    let mock_a = Arc::new(MockReviewProvider::new());
    let mock_b = Arc::new(MockReviewProvider::new());
    let mock_a_dyn: Arc<dyn ReviewProvider> = mock_a.clone();
    // Helper takes Arc<MockReviewProvider> (typed) — pass the typed handle;
    // mock_a_dyn exists only for the ptr_eq assertions below.
    let (ps, _emitter) =
        build_ps_with_clipboard_and_review(Arc::new(MockClipboard::new()), mock_a.clone());

    let previous = ps.delivery().swap_delivery_review(mock_b.clone());
    assert!(Arc::ptr_eq(&previous, &mock_a_dyn));
    ...
    // Behavior routed through the DeliveryController's own handle.
    assert_eq!(mock_b.get_text(), Some("routed text".to_string()));
    // The PipelineState slot never saw the store_text.
    assert_eq!(mock_a.get_text(), None);
}

#[test]
fn swap_delivery_review_moves_only_delivery_side_handle() {
    let mock_a = Arc::new(MockReviewProvider::new());
    let mock_a_dyn: Arc<dyn ReviewProvider> = mock_a.clone();
    let (ps, _emitter) =
        build_ps_with_clipboard_and_review(Arc::new(MockClipboard::new()), mock_a.clone());
    ...
    let previous = ps.delivery().swap_delivery_review(mock_b);
    assert!(Arc::ptr_eq(&previous, &mock_a_dyn));
    // PipelineState accessor still points at mock_A — only the delivery-side
    // handle moved. This also locks the construction invariant the
    // cfg(not(test)) debug_assert guards in production: both handles share
    // the same Arc after PipelineState::new.
    assert!(Arc::ptr_eq(&ps.review(), &mock_a_dyn));
}
